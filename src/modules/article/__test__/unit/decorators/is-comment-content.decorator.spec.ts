...
describe('@IsCommentContent', () => {
  ...
  it('댓글 내용이 1글자인 경우 INVALID_COMMENT_CONTENT 에러가 발생한다', async () => {
  ...
  it('댓글 내용이 2글자인 경우 유효성 검사를 통과한다', async () => {
    const test = new TestClass('va');
    const errors = await validate(test);
    expect(errors.length).toBe(0);
  });

  it('댓글 내용이 256글자인 경우 INVALID_COMMENT_CONTENT 에러가 발생한다', async () => {
