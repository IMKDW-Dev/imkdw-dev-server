...
describe('@IsCategoryDesc', () => {
  ...
  it('카테고리 이름이 1자인 경우 INVALID_CATEGORY_NAME 에러가 발생한다', async () => {
  ...
  it('카테고리 이름이 2자인 경우 유효성 검사를 통과한다', async () => {
    const test = new TestClass('vv');
    const errors = await validate(test);
    expect(errors.length).toBe(0);
  });

  it('카테고리 이름이 21자인 경우 INVALID_CATEGORY_NAME 에러가 발생한다', async () => {
    const test = new TestClass('v'.repeat(21));
    const errors = await validate(test);
    expect(errors[0].constraints.IsCategoryName).toBe(BAD_REQUEST_EXCEPTIONS.INVALID_CATEGORY_NAME);
  });

  it('카테고리 이름이 20자인 경우 유효성 검사를 통과한다', async () => {
